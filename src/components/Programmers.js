...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    // return pioneers[featuredProgrammerId - 1].name;
    return pioneers.find((pioneer) => pioneer.id === featuredProgrammerId).name;
  };

  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: `${featuredProgrammerId ? "gold" : "royalblue"}`, ///? which id's or other?
      ...
      <div className="programmers">
        {
          ...
          pioneers.map((dev) => (
            <div className="programmer" key={dev.id}>
              ...
              <button
                onClick={() => {
                  setFeaturedProgrammerId(dev.id); /* in here set the featured id to be dev.id */
                }}
              >
                Feature
              </button>
      ...
      <div id="featured" style={style}>
        {
          ...
          featuredProgrammerId ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : "Pick an awesome programmer"
        }
      </div>
    </div>
  );
